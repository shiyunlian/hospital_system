...
async function run() {
  ...
  try {
    ...
    const result = await connection.execute(
      `SELECT p.patientId ,p.firstName,p.lastName,p.dob, p.gender,p.diagnosis, p.hospitalizedDate, p.dischargedDate, b.amount from patient p, staffHasPatient s, bill b where p.patientId = s.patientId and p.billId=b.billId`
    );
